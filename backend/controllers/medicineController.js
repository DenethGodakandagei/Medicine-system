...
export const getMedicines = async (req, res) => {
  try {
    const medicines = await Medicine.find().populate("pharmacist", "pharmacyName licenseNumber");
    res.json(medicines);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const getMedicineById = async (req, res) => {
  try {
    const medicine = await Medicine.findById(req.params.id).populate("pharmacist", "pharmacyName licenseNumber");
...
export const updateMedicine = async (req, res) => {
  try {
    ...
    if (!pharmacist || medicine.pharmacist.toString() !== pharmacist._id.toString()) {
