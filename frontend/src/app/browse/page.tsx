...
export default function BrowseMedicines() {
  ...
  const handleRequestMedicine = (medicineId: string): void => {
    setRequestedMedicines((prev) => {
      const updated = prev.includes(medicineId)
        ...
        : [...prev, medicineId];
      console.log("Requested Med (after update):", updated);
      return updated;
    });
  };
  ...
  const handleSendRequests = async () => {
    ...
    try {
      const response = await API.post("/requests/my", {
        medicineIds: requestedMedicines,
      });

      toast.success("Medicine requests sent successfully!");
      setRequestedMedicines([]);
    } catch (error: any) {
      if (error.response?.status === 401) {
        toast.error("You must log in to request medicines.");
        router.push("/login");
        return;
      }
          ...
          <>
            ...
            {requestedMedicines.length > 0 && (
              <div className="p-4 mb-6 border border-blue-100 rounded-lg bg-blue-50">
                ...
                >
                  {user
                    ? `Send Requests (${requestedMedicines.length})`
                    : "Login to Request"}
                </button>
              </div>
            )}
